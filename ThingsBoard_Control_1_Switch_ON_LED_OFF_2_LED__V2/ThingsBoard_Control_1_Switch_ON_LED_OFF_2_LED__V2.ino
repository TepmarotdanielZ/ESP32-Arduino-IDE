...
void mqttCallback(char* topic, byte* payload, unsigned int length) {  
  ...
  if (message.indexOf("\"params\":true") != -1) {  
    /* WHEN PARAMS IS TRUE, TURN LEDS ON */  
    digitalWrite(led1Pin, LOW); /* TURN LED 1 ON */  
    digitalWrite(led2Pin, LOW); /* TURN LED 2 ON */ 
    Serial.println("LED 1 and LED 2 State: ON");  
    publishLEDState("ON"); /* PUBLISH LED STATE */   
  } else if (message.indexOf("\"params\":false") != -1) {  
    /* WHEN PARAMS IS FALSE, TURN LEDS OFF */   
    digitalWrite(led1Pin, HIGH); /* TURN LED 1 OFF */   
    digitalWrite(led2Pin, HIGH); /* TURN LED 2 OFF */   
    Serial.println("LED 1 and LED 2 State: OFF");  
    publishLEDState("OFF"); /* PUBLISH LED STATE */  
  }   
}  

void publishLEDState(const char* state) {  
  /* PUBLISH THE LED STATE TO THINGSBOARD  */  
  String payload = "{\"led1\": \"" + String(state) + "\", \"led2\": \"" + String(state) + "\"}";   
