...
int pirValue = LOW;
String pirStatus = "No motion detected";
...
void taskPrintSensors(void *pvParameters) {
  ...
  while (true) {
    ...
    pirStatus = (pirValue == HIGH) ? "No Motion " : "Motion Detected";
    ...
    snprintf(payload, sizeof(payload),
             ...
             "\"uv_voltage\":%.2f,\"uv_intensity\":%.1f,\"pir_status\":\"%s\"}",
             isnan(humidity) ? 0.0 : humidity,
             isnan(temperature) ? 0.0 : temperature,
             uvRawValue, uvVoltage, uvIntensity, pirStatus.c_str());
...
void taskReadNFC(void *pvParameters) {
  ...
  while (true) {
    if (nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength)) {
      nfcFlag++;
      digitalWrite(LED_PIN, (nfcFlag == 1) ? HIGH : LOW);
